...
int main(int argc, char **argv) {
  ...
  for (int iGeometry = 0; iGeometry < maxNumOfGeo; iGeometry++) {
    ...
    switch (iGeometry) {
    case 0:
      f = vector<double>(10 * cos(10 * x<double>()), 0., 0.);
      break;
    case 1:
    case 2:
      f = vector<double>(10 * cos(10 * x<double>()), 10 * cos(10 * y<double>()),
                         0.);
      break;
    case 3:
    case 4:
    ...
    PGFPlotsio latexPlot("GradH1errorPlotFor" + elementType[iGeometry]);
    for (int iFEorder = 1; iFEorder <= FEorderMax; iFEorder++) {
      ...
      for (int i = 1; i <= numberOfPoints; i++) {
        ...
        Field<double, Form::Form0> v(
            "v", omega, FunctionSpaceTypeForm0::HierarchicalH1, iFEorder);

        formulation.integral(grad(dof(v)), grad(tf(v)), omega, gauss);
        formulation.integral(-f, grad(tf(v)), omega, gauss);

        formulation.pre();
        formulation.assemble();
        formulation.solve();
        double numx =
            integrate(pow(abs(xComp(f) - xComp(grad(v))), 2), omega, gauss);
        ...
        case 2:
          error +=
              integrate(pow(abs(yComp(f) - yComp(grad(v))), 2), omega, gauss) /
        ...
        case 5:
          error +=
              integrate(pow(abs(yComp(f) - yComp(grad(v))), 2), omega, gauss) /
                  integrate(pow(abs(yComp(f)), 2), omega, gauss) +
              integrate(pow(abs(zComp(f) - zComp(grad(v))), 2), omega, gauss) /
  ...
  it = 0;
  msg::info
      << " In this code, the developed conforming hierarchical finiteelement "
         "approximations for the space GradH1 is  validated."
      << msg::endl;
