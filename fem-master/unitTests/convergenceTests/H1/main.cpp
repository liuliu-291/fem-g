...
int main(int argc, char **argv) {
  ...
  for (int iGeometry = 0; iGeometry < maxNumOfGeo; iGeometry++) {
    ...
    case 1:
    case 2:
      f = sin(10. * x<double>()) + sin(10. * y<double>());
      break;
    case 3:
    case 4:
    case 5:
    ...
    PGFPlotsio latexPlot("H1errorPlotFor" + elementType[iGeometry]);
    for (int iFEorder = 1; iFEorder <= FEorderMax; iFEorder++) {
      ...
      for (int i = 1; i <= numberOfPoints; i++) {
        ...
        double num = integrate(pow(abs(f - v), 2), omega, gauss);
        double den = integrate(pow(abs(f), 2), omega, gauss);
  ...
  msg::info << "********************************" << msg::endl;
  it = 0;
  msg::info << " In this code, the developed conforming hierarchical finite "
               "element approximations for the space H1 is validated."
            << msg::endl;
