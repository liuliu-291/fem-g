...
namespace gmshfem::domain
{
  ...
  SphericalShell< T_PScalar >::SphericalShell(const T_PScalar Rmin, const T_PScalar Rmax, const T_PScalar centerX, const T_PScalar centerY, const T_PScalar centerZ, const unsigned int exponant) :
    _Rmin(Rmin), _Rmax(Rmax), _center{centerX, centerY, centerZ}, _exponant(exponant)
  {
  }

  template< class T_PScalar >
  SphericalShell< T_PScalar >::~SphericalShell()
  {
  }

  template< class T_PScalar >
  bool SphericalShell< T_PScalar >::needGaussCoordinates() const
  {
    return true;
  }

  template< class T_PScalar >
  void SphericalShell< T_PScalar >::apply(const std::vector< T_PScalar, numa::allocator< T_PScalar > > &points, std::vector< T_PScalar, numa::allocator< T_PScalar > > &determinants, std::vector< T_PScalar, numa::allocator< T_PScalar > > &jacobians) const
  {
...
#pragma omp for
    for(auto i = 0ULL; i < nbrValues; ++i) {
      ...
      if(R < _Rmin - 10. * scalar::Epsilon< T_PScalar >::value || R > _Rmax + 10. * scalar::Epsilon< T_PScalar >::value) {
        throw common::Exception("Wrong paramameters for 'SphericalShell': the integration point radius is " + std::to_string(R) + " whereas the shell boundaries are (" + std::to_string(_Rmin) + ", " + std::to_string(_Rmax) + ")");
      }
  ...
  template< class T_PScalar >
  SphericalShell< T_PScalar > *SphericalShell< T_PScalar >::copy() const
  {
    return new SphericalShell< T_PScalar >(_Rmin, _Rmax, _center[0], _center[1], _center[2], _exponant);
  }

  INSTANTIATE_CLASS(SphericalShell, 2, TEMPLATE_ARGS(double, float))
