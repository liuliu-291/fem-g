...
int main(int argc, char **argv)
{
  ...
  switch(runcase) {
  case 1:
    abcName.push_back("ABC-0");
    abcName.push_back("ABC-2");
    abcName.push_back("ABC-22");
    abcName.push_back("ABC-23");
    abcName.push_back("ABC-24");
  ...
  for(int j = 0; j <= 300; j++) {
    ...
    for(unsigned int j = 0; j <= (abcName.size() - 1); j++) {
      ...
      if(abcName[j] == "ABC-0") {
      ...
      else if(abcName[j] == "ABC-23") {
        msg::info << "Use second order ABC with 3 symbols." << msg::endl;
        formulation.integral(im * k0 * dof(*vNum[j]), tf(*vNum[j]), gammaRight, gauss);
        formulation.integral(-im / (2. * k0) * grad(dof(*vNum[j])), grad(tf(*vNum[j])), gammaRight, gauss);
        formulation.integral(dx_c0_m2 / (4. * c0_m2) * dof(*vNum[j]), tf(*vNum[j]), gammaRight, gauss);
        formulation.integral(dx_c0_m2 / (4. * k0 * k0) * grad(dof(*vNum[j])), grad(tf(*vNum[j])), gammaRight, gauss);
        formulation.integral(im * 5. * pow(dx_c0_m2, 2) / (32. * k0 * pow(c0_m2, 2)) * dof(*vNum[j]), tf(*vNum[j]), gammaRight, gauss);
        formulation.integral(im * 25. * pow(dx_c0_m2, 2) / (64. * pow(k0, 3) * pow(c0_m2, 2)) * grad(dof(*vNum[j])), grad(tf(*vNum[j])), gammaRight, gauss);
      }
      else if(abcName[j] == "ABC-24") {
        ...
        formulation.integral(im * 5. * pow(dx_c0_m2, 2) / (32. * k0 * pow(c0_m2, 2)) * dof(*vNum[j]), tf(*vNum[j]), gammaRight, gauss);
        formulation.integral(im * 25. * pow(dx_c0_m2, 2) / (64. * pow(k0, 3) * pow(c0_m2, 2)) * grad(dof(*vNum[j])), grad(tf(*vNum[j])), gammaRight, gauss);
        formulation.integral(-15. * pow(dx_c0_m2, 3) / (64. * pow(k0, 2) * pow(c0_m2, 3)) * dof(*vNum[j]), tf(*vNum[j]), gammaRight, gauss);
        formulation.integral(-15. * pow(dx_c0_m2, 3) / (16. * pow(k0, 4) * pow(c0_m2, 3)) * grad(dof(*vNum[j])), grad(tf(*vNum[j])), gammaRight, gauss);
      }
        ...
        if(abcName[j] == "Pade-S2") {
          ...
          formulation.integral(-(w * w) * (c0_m2)*dof(*psi[0]), tf(*psi[0]), gammaRight, gauss);
          formulation.integral((w * w) * (c0_m2)*dof(*vNum[j]), tf(*psi[0]), gammaRight, gauss);
        }
      }
